...
function confirmLogout() {
  ...
  <div class="relative">
    ...
    <h2 class="text-2xl font-semibold text-blue-700">Kelola Seminar</h2>

    <div class="flex items-center flex-1 justify-between ml-10 space-x-10">
        <!-- Form Pencarian -->
        <form action="<?= base_url('seminar/search') ?>" method="GET" class="flex flex-grow max-w-xl">
            <input
                type="text"
                name="query"
        ...
        <a href="<?= base_url('seminar/input') ?>" 
           class="bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-800 transition flex items-center gap-2">
...
<section class="bg-white rounded-lg shadow p-5 overflow-auto">
   <div class="flex justify-end mb-4 space-x-3">
    ...
    <a href="<?= base_url('laporan/laporan_seminar_pdf'); ?>"  class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition"><i class="far fa-file-pdf"></i> Download Pdf</a> 

    <a href="<?= base_url('laporan/export_excel'); ?>"  class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition"><i class="far fa-file-excel"></i> Download Excel</a>
    ...
    <table class="min-w-full text-left text-sm">
        <thead class="bg-blue-50 text-blue-700 font-semibold">
            <tr>
                ...
                <th class="px-3 py-2 border-b border-gray-200">Narasumber</th>
    ...
    <?php else: ?>
        <?php foreach ($seminars as $seminar): ?>
            <tr class="hover:bg-blue-50">
                <td class="px-3 py-2 border-b border-gray-200"><?= $seminar->id_seminar ?></td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= htmlspecialchars($seminar->judul) ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= htmlspecialchars($seminar->kategori) ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= htmlspecialchars($seminar->lokasi) ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= date('d M Y', strtotime($seminar->tanggal)) ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= date('H:i', strtotime($seminar->waktu)) ?>
                </td>
               <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= $seminar->harga == 0 ? 'Free' : 'Rp ' . number_format($seminar->harga, 0, ',', '.') ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 <?= (strtotime($seminar->tanggal) < time()) ? 'text-red-600' : '' ?>">
                    <?= htmlspecialchars($seminar->narasumber ?? '-') ?>
                </td>
                <td class="px-3 py-2 border-b border-gray-200 space-x-2">
                    <a href="<?= base_url('seminar/edit/' . $seminar->id_seminar) ?>" class="text-blue-600 hover:text-blue-800" title="Edit Seminar">
                    ...
                    title="Hapus Seminar" 
                    data-id="<?= $seminar->id_seminar ?>">
                    <i class="fas fa-trash-alt"></i>
                  </button>

                    <a href="<?= base_url('seminar/detail/' . $seminar->id_seminar) ?>" class="text-blue-600 hover:text-blue-800" title="Detail Seminar">
                        <i class="fas fa-info-circle"></i>
                    </a>
                    <a href="<?= base_url('transaksi/detailpeserta/' . $seminar->id_seminar) ?>" class="text-green-600 hover:text-green-800" title="Detail Peserta">
                        <i class="fas fa-users"></i>
                    </a>
                </td>
            </tr>
