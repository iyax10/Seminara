...
<head>
  ...
  <title>SEMINARA Admin - Data Transaksi</title>
 ...
 <aside class="w-64 bg-white shadow-md flex flex-col">
    ...
    <nav class="flex flex-col flex-grow px-4 py-6 space-y-2 text-gray-700 text-sm">
     <a href="<?= base_url('admin') ?>" class="flex items-center gap-3 px-3 py-2 rounded hover:bg-blue-50 hover:text-blue-700 transition">
      ...
      <a href="<?= base_url('Transaksi') ?>" class="flex items-center gap-3 px-3 py-2 rounded bg-blue-50 text-blue-700 font-semibold">
...
function confirmLogout() {
  ...
  <div class="relative">
    ...
    <main class="p-6 space-y-8 overflow-auto">
      <h2 class="text-2xl font-semibold text-blue-700">Data Transaksi</h2>

      <section class="bg-white rounded-lg shadow p-5 overflow-auto">
         <div class="flex justify-end mb-4 space-x-3">
       <a href="<?= base_url('laporan/laporan_transaksi_pdf'); ?>"  class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition"><i class="far fa-file-pdf"></i> Download Pdf</a> 

    <a href="<?= base_url('laporan/export_excel_transaksi'); ?>"  class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition"><i class="far fa-file-excel"></i> Download Excel</a>
        </div>
        ...
        <table class="min-w-full text-left text-sm">
          <thead class="bg-blue-50 text-blue-700 font-semibold">
            <tr>
              ...
              <th class="px-3 py-2 border-b border-gray-200">Email Pengguna</th>
              <th class="px-3 py-2 border-b border-gray-200">No. Tlp</th>
              <th class="px-3 py-2 border-b border-gray-200">Seminar</th>
              <th class="px-3 py-2 border-b border-gray-200">Tanggal Beli</th>
              <th class="px-3 py-2 border-b border-gray-200">Harga</th>
              <th class="px-3 py-2 border-b border-gray-200">Status</th>
              <th class="px-3 py-2 border-b border-gray-200">Kode Tiket</th>
              <!-- <th class="px-3 py-2 border-b border-gray-200">Aksi</th> -->
            </tr>
          </thead>
          <tbody>
            <?php if (empty($transaksi)): ?>
              <tr>
                <td colspan="8" class="px-3 py-2 border-b border-gray-200 text-center">Belum ada transaksi</td>
              </tr>
            <?php else: ?>
              <?php foreach ($transaksi as $item): ?>
                <tr class="hover:bg-blue-50">
                  <td class="px-3 py-2 border-b border-gray-200"><?= $item->id_transaksi ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= $item->nama_pengguna ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= $item->email_pengguna ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= $item->tlp_pengguna ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= $item->judul_seminar ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= date('Y-m-d H:i', strtotime($item->tanggal_transaksi)) ?></td>
                  <td class="px-3 py-2 border-b border-gray-200"><?= number_format($item->harga, 2) ?></td>
                  <td class="px-3 py-2 border-b border-gray-200">
                    <span class="inline-block px-2 py-0.5 rounded text-xs font-semibold <?= $item->status == 'pending' ? 'text-white bg-yellow-500  ' : ($item->status == 'berhasil' ? 'text-white bg-green-600' : 'text-white bg-red-500') ?>"> 
                      <?= ucfirst($item->status) ?>
                    </span>
                  </td>
                      <td class="px-3 py-2 border-b border-gray-200">
                          <?= $item->status == 'berhasil' ? $item->kode_tiket : 'Belum ada kode tiket' ?>
                      </td>
                  <td class="px-3 py-2 border-b border-gray-200 space-x-2">
                    <!-- <button class="text-blue-600 hover:text-blue-800" title="Detail Transaksi">
                      <i class="fas fa-info-circle"></i>
                    </button> -->
                    <?php if ($item->status == 'pending'): ?>
                      <button class="text-green-600 hover:text-green-800" title="Konfirmasi Transaksi" onclick="konfirmasiTransaksi(<?= $item->id_transaksi ?>)">
  ...
  <script>
    function konfirmasiTransaksi(id_transaksi) {
      Swal.fire({
        title: 'Apakah Anda yakin ingin mengonfirmasi transaksi ini?',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          window.location.href = '<?= base_url("transaksi/konfirmasi/") ?>' + id_transaksi;
        }
      });
    }
