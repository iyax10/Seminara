  ...
  <?php foreach ($favorit as $fav) : ?>
    <div class="rounded-md border border-gray-200 mb-6" >
      <div class="flex items-center justify-between bg-gray-50 px-4 py-2 text-sm text-gray-900 border-b border-gray-200">
        <p>
          Tanggal : <span class="font-normal"><?= date('d M Y', strtotime($fav['tanggal'])); ?></span>
          <span class="mx-2">|</span>
          Waktu : <span class="font-normal"><?= $fav['waktu']; ?></span>
        </p>
      </div>

      <div class="flex items-center justify-between px-4 py-4 border-b border-gray-200">
        <p class="text-gray-700 font-semibold flex-1">
          <a href="<?= base_url('seminar/seminar_info/' . $fav['id_seminar']) ?>" class="hover:underline text-blue-700">
            <?= htmlspecialchars($fav['judul']) ?>
          </a>
        </p>
        <div class="border-l border-gray-300 pl-6">
          <p class="text-gray-500 text-sm">Kategori</p>
          <p class="font-extrabold text-gray-900"><?= htmlspecialchars($fav['kategori']) ?></p>
        </div>
      </div>
    </div>
  <?php endforeach; ?>
<?php else : ?>
  <p class="text-center text-gray-500">Belum ada seminar favorit.</p>
<?php endif; ?>
